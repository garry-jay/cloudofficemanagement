...
public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    /**
     * 添加这个Endpoint,这样就饿可以通过websocket连接服务，让前端连接上websocket服务
     * 也就是我么配置websocket的服务地址,并可以指定是否使用socketJs
     * @param registry
     */
    @Override
...
//        registry.addEndpoint("/ws/ep").setAllowedOrigins("*").withSockJS();
    ...
    public void configureClientInboundChannel(ChannelRegistration registration) {
        registration.interceptors(new ChannelInterceptor() {
            ...
            public Message<?> preSend(Message<?> message, MessageChannel channel) {
                ...
                //判断是不是连接，如果是，需要获取token，并设置到用户对象
    ...
    public void configureMessageBroker(MessageBrokerRegistry registry) {
        //配置代理域，可以配置多个，配置代理目的地前缀为/queue，可以在配置域上向客户端client推送消息
        registry.enableSimpleBroker("/queue");//不是rabbitmq的queue
    }
}
